...
public abstract class Ship extends Actor implements Listener, playn.core.Keyboard.Listener {
	...
	public void drawStarfield(Surface target) {
		if (starfield == null) {
			...
			for (int i = 0; i < 1500; ++i) {
				...
				starfield.canvas().drawPoint(SpaceImperatorGame.s.rand.nextInt((int) starfield.width()), SpaceImperatorGame.s.rand.nextInt((int) starfield.height()));
			}
		}
	...
	public void update() {
		...
		if (!hasTurrets())
			body.applyLinearImpulse(new Vec2((float) Math.cos(body.getAngle() + Math.PI / 2) * this.getThrustPower() * strafe, (float) Math.sin(body.getAngle() + Math.PI / 2) * this.getThrustPower() * strafe),
					body.getPosition());
